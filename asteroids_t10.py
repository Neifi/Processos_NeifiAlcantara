...
class World(object):
    ...
    def handle_keyup(self, event):
        ...
        if event.key == pygame.K_SPACE:
            ...
            bullet = Bullet((Vector(*self.player.rect.center) -
                             self.player.facing*3).to_position(), direction, magnitude)
...
def main():
    ...
    # main loop
    actualizacionColision = Thread(target=update_colision)
    supdate = Thread(target=update_s)
    actualizacionColision.start()
    supdate.start()
    while world.running:
